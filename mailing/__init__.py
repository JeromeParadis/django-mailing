...
# Versioning
# --------------------------------
VERSION = (0, 1, 0, "beta", 0) # following PEP 386
DEV_N = None
...
def send_email(recipients, subject, text_content=None, html_content=None, from_email=settings.DEFAULT_FROM_EMAIL, category=None, fail_silently=False, bypass_queue=False):
    """
    ...
    if not bypass_queue and hasattr(settings, 'MAILING_USE_CELERY') and settings.MAILING_USE_CELERY:
        ...
        if text_content or html_content:
            ...
            msg = EmailMultiAlternatives(subject, text_content if text_content else html_content, from_email, recipients_list, headers = headers)
...
# Monkey-patch core Django email
# --------------------------------
if hasattr(settings,'MAILING_REPLACE_DJANGO_CORE_EMAIL'):
    if settings.MAILING_REPLACE_DJANGO_CORE_EMAIL:
        try:
